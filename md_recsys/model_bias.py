...
# 评分预测    1-5
class MatrixDecomForRecSys_bias(object):
    def __init__(
            self, 
            lr,
            batch_size,
    ...
    def train(self, optimizer_type: str):
        ...
        for i in range(self.epoch):
            ...
            if best_metric_result is None or metric_result <= best_metric_result:
                ...
                best_P, best_Q, best_bu, best_bi = P, Q, bu, bi
                print("Best dev metric result: {}".format(best_metric_result))

        # 最后保存最好的P和Q
        np.savez("best_pq.npz", P=best_P, Q=best_Q, bu=best_bu, bi=best_bi)
    ...
    def sgd(self, P, Q, bu, bi):
        ...
        *********************************
        ...
        for uid, iid, real_rating in self.train_data.itertuples(index=False):
            p_u, q_i = P[uid], Q[iid]
            err = np.float32(real_rating - self.globalMean - bu[uid] - bi[iid] - np.dot(p_u, q_i))
    ...
    def predict_user_item_rating(self, uid, iid, P, Q, bu, bi):
        ...
        return self.globalMean + np.dot(p_u,q_i) + bu[uid] + bi[iid]
    
    def eval(self, P, Q, bu, bi):
        ...
        for uid, iid, real_rating in self.dev_data.itertuples(index=False):
            prediction_rating = self.predict_user_item_rating(uid, iid, P, Q, bu, bi)
    ...
    def test(self, test_data):
        ...
        P, Q, bu, bi = best_pq["P"][()], best_pq["Q"][()], best_pq["bu"][()], best_pq["bi"][()]

        save_results = list()
        for _, uid, iid in test_data.itertuples(index=False):
            pred_rating = self.predict_user_item_rating(uid, iid, P, Q, bu, bi)
